...
\begin{itemize}
  ...
  例如在模拟CPU时，只有在取指、译码、执行等事件发生时才会进行模拟，其余时间Gem5处于“休眠状态”，这样可以提高模拟器的效率。
\end{itemize}
...
为了和真实的硬件环境更加接近，我们使用了和真实硬件校准过的Intel Haswell处理器参数进行模拟，
\cite{akramValidationGem5Simulator2019}校准后的数据表明，Gem5模拟器和真实硬件的性能差距在6\%以内。
我们将这个参数作为基准参数，然后在这个基准参数上进行微译器的实验。
...
\begin{table}[ht]
  ...
  \multirow{6}{*}{\textbf{处理器核}} & \multicolumn{1}{l|}{软件动静翻译+融合微码}           & 硬件译码 + X86微码    \\ \cline{2-3} 
